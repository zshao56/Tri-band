...
def sim_plot(sim, folder_name):
    # fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 4), gridspec_kw={'width_ratios': [3, 1]})
    # sim.plot(y=0, ax=ax1)
    # ax1.set_xlim(-0.5, 0.5)
    # ax1.set_xlabel('X (µm)')
    # ax1.set_aspect('auto')  # 这允许x轴被拉伸
    # sim.plot(z=0, ax=ax2)
    # plt.tight_layout()
    # plt.savefig(f'./results/SPE_WO3/sim_structure_wwo3{w_WO3}_tSPE{t_SPE}.png')
    ...
    plt.savefig(data_path(f'{folder_name}/grid_sim_structure_wwo3{w_WO3}_tSPE{t_SPE}3.png'))
    plt.close() 
...
def plot_combined_results(sim_data_B, sim_data_C, w_WO3, t_SPE, folder_name):
    ...
    plt.grid(True)
    plt.savefig(data_path(f"{folder_name}/results_wwo3{w_WO3}_tSPE{t_SPE}3.png"))
    plt.close() 




    output_file = data_path(f"{folder_name}/results_wwo3{w_WO3}_tSPE{t_SPE}3.txt")
...
w_WO3 = 0.45
t_SPE = 0.55
folder_name = './results/ITO_top-WO3'
